IntercomRails.config do |config|
  ...
  config.user.custom_data = {
    ...
    email: :email_address,
    phone: :phone_number,
    created_at: Proc.new { |user| user.created_at.to_i },
    on_idt_two: Proc.new { true },
    user_hash:  Proc.new { |user| "#{ OpenSSL::HMAC.hexdigest('sha256', 'FK6pAyAjs9x5GePcTHsH7j0J-Wj4M-aKUOWV1W1C', user.email_address) }" }
  }
  ...
  config.company.custom_data = {
    company_id: :id,
    name: :name_or_something_else_identifying,
    created_at: Proc.new { |organization| organization.created_at.to_i },
    plan: Proc.new { |organization| organization.plan_stripe_id }
  }
  ...
  #
  # config.company.plan = Proc.new { |organization| organization.plan_stripe_id }
